...
gpt_tokenizer = GPT2Tokenizer.from_pretrained("/home/sichenglei/gpt_tokenizer")
...
def PromptStep(args, prompt, temp):
    ...
    if "code" in args.engine and len(gpt_tokenizer.tokenize(prompt)) > 7900:
        return "", prompt
    
    if "text" in args.engine and len(gpt_tokenizer.tokenize(prompt)) >= 2000:
        return "", prompt
...
def main():
    ...
    parser.add_argument('--prompt_method', type=str, default=None, help='specify the prompting method', choices=["zeroshot", "zeroshot-step", "fewshot", "fewshot-cot", "fewshot-cot-selfcon"])
    ...
    for eg in tqdm(test_set):
        ...
        ## few-shot demo
        if args.prompt_method in ["fewshot", "fewshot-cot", "fewshot-cot-selfcon"]:
        ...
        if args.task not in ["subqa-goldsub1", "subqa-goldqa1", "subqa-step1"]:
            prompt += "Answer: "
        ...
        if args.prompt_method == "fewshot-cot-selfcon":
